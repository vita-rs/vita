...
/// interchange, so the constitution admits more than one configuration.
///
...
/// A locus survives when the substituent classes its geometry admits fill its slots
/// in more than one distinct way, so repeated substituents are weighed correctly: an
...
///
/// A site could bear stereochemistry where its
/// [`coordination_geometry`](HasCoordinationGeometries::coordination_geometry) admits
/// more than one configuration, a bond and an axis where the graph gives them a rigid
/// frame. This keeps only those a symmetry does *not* neutralise: `site_key` and
/// `bond_key` color the graph as for [`canonicalize`](crate::canonical::canonicalize),
/// the declared configurations refine that coloring to a fixpoint — so a center whose
/// stereogenicity turns on its neighbors' handedness is resolved — and a locus
...
/// [`canonicalize`](crate::canonical::canonicalize) per candidate locus.
    ...
    EK: Ord,
{
    ...
    let surviving = |locus| (realizable(mol, locus, &key, &bond_key).len() > 1).then_some(locus);
...
mod tests {
    ...
    fn detect(mol: &Mol) -> Stereocenters {
        stereocenters(mol, |site| mol.color(site), |_| 0u32)
    }
    ...
    fn symmetry_equivalent_centers_are_each_detected() {
        let centers =
            detect(&paired_centers().with_geometries([(1, Tetrahedral), (2, Tetrahedral)]));
        assert!(centers.contains(StereoLocus::Site(s(1))));
    ...
    fn a_pseudo_asymmetric_center_is_detected() {
        ...
        let centers = detect(&mol.with_geometries([(2, Tetrahedral)]));
        assert!(centers.contains(StereoLocus::Site(s(2))));
    }
    ...
    fn detection_is_independent_of_input_order() {
        let molecule = paired_centers().with_geometries([(1, Tetrahedral), (2, Tetrahedral)]);
        assert_eq!(detect(&molecule), detect(&reversed(&molecule)));
    }
}
