...
impl<M: HasBonds> Iterator for Dfs<'_, M> {
    type Item = SiteId;

    fn next(&mut self) -> Option<SiteId> {
        let site = self.frontier.pop()?;
        let mol = self.mol;
        let base = self.frontier.len();
        for neighbor in mol.neighbors(site) {
            if self.visited.insert(neighbor) {
                self.frontier.push(neighbor);
            }
        }
        self.frontier[base..].reverse();
        Some(site)
    }
...
mod tests {
    ...
    fn single_site_yields_only_itself() {
        assert_eq!(dfs(&single(), s(1)).collect::<Vec<_>>(), vec![s(1)]);
    }

    #[test]
    fn start_is_yielded_first() {
        assert_eq!(dfs(&chain(), s(2)).next(), Some(s(2)));
        assert_eq!(dfs(&chain(), s(3)).next(), Some(s(3)));
    }

    #[test]
    fn reaches_every_site_in_a_connected_molecule() {
        let mut reached: Vec<SiteId> = dfs(&tree(), s(1)).collect();
    ...
    fn does_not_reach_other_components() {
        let reached: Vec<SiteId> = dfs(&disconnected(), s(1)).collect();
        assert!(!reached.contains(&s(3)));
    }

    #[test]
    fn isolated_start_yields_only_itself() {
        assert_eq!(dfs(&disconnected(), s(3)).collect::<Vec<_>>(), vec![s(3)]);
    }

    #[test]
    fn explores_a_branch_fully_before_a_sibling() {
        let order: Vec<SiteId> = dfs(&tree(), s(1)).collect();
        let position = |x: SiteId| order.iter().position(|&v| v == x).unwrap();
        assert!(position(s(4)) < position(s(3)));
        assert!(position(s(5)) < position(s(3)));
    }

    #[test]
    fn visits_a_cycle_without_revisiting_a_site() {
        let order: Vec<SiteId> = dfs(&triangle(), s(1)).collect();
        let unique: FxHashSet<SiteId> = order.iter().copied().collect();
        assert_eq!(order.len(), unique.len());
    }

    #[test]
    fn descends_a_path_from_an_endpoint() {
        assert_eq!(
    ...
    #[test]
    fn takes_branches_in_the_order_their_bonds_are_reached() {
        assert_eq!(
    ...
    fn counts_the_sites_in_the_reachable_component() {
        ...
        assert_eq!(dfs(&disconnected(), s(1)).count(), 2);
    }

    #[test]
    fn take_yields_only_the_requested_prefix() {
        assert_eq!(
            dfs(&chain(), s(1)).take(2).collect::<Vec<_>>(),
            vec![s(1), s(2)],
        );
    }
