...
mod tests {
    ...
    #[test]
    fn rem() {
        let r = Energy::<f64, Hartree>::new(7.0) % Energy::new(3.0);
        assert_eq!(r.value(), 1.0);
    }

    #[test]
    fn rem_assign() {
        let mut e = Energy::<f64, Hartree>::new(7.0);
        e %= Energy::new(3.0);
    ...
    #[test]
    fn rem_scalar() {
        let r = Energy::<f64, Hartree>::new(7.0) % 3.0;
        assert_eq!(r.value(), 1.0);
    }
    ...
    #[test]
    fn signum() {
        assert_eq!(Energy::<f64, Hartree>::new(3.0).signum(), 1.0);
        assert_eq!(Energy::<f64, Hartree>::new(-3.0).signum(), -1.0);
    }

    #[test]
    fn copysign() {
        let e = Energy::<f64, Hartree>::new(3.0);
        let sign = Energy::<f64, Hartree>::new(-1.0);
        assert_eq!(e.copysign(sign).value(), -3.0);
        assert_eq!((-e).copysign(e).value(), 3.0);
    }

    #[test]
    fn floor() {
        assert_eq!(Energy::<f64, Hartree>::new(2.7).floor().value(), 2.0);
        assert_eq!(Energy::<f64, Hartree>::new(-2.3).floor().value(), -3.0);
    }

    #[test]
    fn ceil() {
        assert_eq!(Energy::<f64, Hartree>::new(2.3).ceil().value(), 3.0);
        assert_eq!(Energy::<f64, Hartree>::new(-2.7).ceil().value(), -2.0);
    }

    #[test]
    fn round() {
        assert_eq!(Energy::<f64, Hartree>::new(2.5).round().value(), 3.0);
        assert_eq!(Energy::<f64, Hartree>::new(-2.5).round().value(), -3.0);
    }
    ...
    fn round_ties_even() {
        assert_eq!(
            Energy::<f64, Hartree>::new(2.5).round_ties_even().value(),
            2.0
        );
        assert_eq!(
            Energy::<f64, Hartree>::new(3.5).round_ties_even().value(),
            4.0
        );
    }
    ...
    #[test]
    fn trunc() {
        assert_eq!(Energy::<f64, Hartree>::new(2.7).trunc().value(), 2.0);
        assert_eq!(Energy::<f64, Hartree>::new(-2.7).trunc().value(), -2.0);
    }

    #[test]
    fn fract() {
        assert!((Energy::<f64, Hartree>::new(2.75).fract().value() - 0.75).abs() < 1e-12);
    }

    #[test]
    fn div_euclid() {
        let q = Energy::<f64, Hartree>::new(7.0).div_euclid(Energy::new(3.0));
        assert_eq!(q, 2.0);
    }

    #[test]
    fn rem_euclid() {
        let r = Energy::<f64, Hartree>::new(-7.0).rem_euclid(Energy::new(3.0));
        assert_eq!(r.value(), 2.0);
    }

    #[test]
    fn mul_add() {
        let r = Energy::<f64, Hartree>::new(2.0).mul_add(3.0, Energy::new(1.0));
        assert_eq!(r.value(), 7.0);
    }

    #[test]
    fn hypot() {
        let h = Energy::<f64, Hartree>::new(3.0).hypot(Energy::new(4.0));
        assert!((h.value() - 5.0).abs() < 1e-12);
    }

    #[test]
    fn is_nan() {
        assert!(Energy::<f64, Hartree>::new(f64::NAN).is_nan());
        assert!(!Energy::<f64, Hartree>::new(1.0).is_nan());
    }

    #[test]
    fn is_infinite() {
        assert!(Energy::<f64, Hartree>::new(f64::INFINITY).is_infinite());
        assert!(!Energy::<f64, Hartree>::new(1.0).is_infinite());
    }

    #[test]
    fn is_finite() {
        assert!(Energy::<f64, Hartree>::new(1.0).is_finite());
        assert!(!Energy::<f64, Hartree>::new(f64::INFINITY).is_finite());
        assert!(!Energy::<f64, Hartree>::new(f64::NAN).is_finite());
    }

    #[test]
    fn is_sign_positive() {
        assert!(Energy::<f64, Hartree>::new(1.0).is_sign_positive());
        assert!(!Energy::<f64, Hartree>::new(-1.0).is_sign_positive());
    }

    #[test]
    fn is_sign_negative() {
        assert!(Energy::<f64, Hartree>::new(-1.0).is_sign_negative());
        assert!(!Energy::<f64, Hartree>::new(1.0).is_sign_negative());
    }
